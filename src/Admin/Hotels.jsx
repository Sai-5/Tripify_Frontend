...
const Hotels = () => {
  ...
  const deleteData = (taskId) => {
    axios.delete(`http://localhost:7000/deletehotel/${taskId}`);
  ...
  return (
    <div>
      ...
      >
        <Button onClick={() => navigate("/createhotel")}>Add Hotel</Button>
      </div>
          ...
          <tbody>
            {items.map((item, index) => (
              <tr key={item._id}>
                ...
                <td>
                  Normal Suite: {item.price.normal_suite} <br />
                  VIP Suite: {item.price.vip_suite}
                </td>
                <td>
                  <div
